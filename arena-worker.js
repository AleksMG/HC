...
// ALL FEATURES INCLUDED — NO SIMPLIFICATIONS
// ============================================================================
...
class SixteenHeadAttention {
    constructor(dim, password) {
        this.dim = dim;
        this.numHeads = 16;
        this.headDim = Math.floor(dim / this.numHeads);        const rng = new SeededRandom(password + "_ATTENTION_16H");
        this.W_q = [];
        this.W_k = [];
        this.W_v = [];
...
class HCAgent {
    constructor(password, id) {
        ...
        this.blocks = 12;
        this.trajectory = [];
        this.maxTrajectory = 50;        this.maxReverseSteps = 10;
    ...
    encodeInput(input) {
        ...
        for (let i = 0; i < this.workingDim; i++) {
            ...
            for (let j = 0; j < input.length && j < 32; j++) {
                if (typeof input[j] === 'number' && isFinite(input[j])) {                    sum += input[j] * this.W_input[i][j];
                }
            }
            state.push(tanh(sum));
        }
    ...
    reverse(steps) {
        ...
        this.trajectory.splice(-steps);        this.mutations++;
    ...
    setWeights(weights) {
        ...
        this.W_output = deepClone(weights.output);        if (weights.metadata) {
...
// ============================================================================
// MESSAGE HANDLER — ALL MESSAGE TYPES HANDLED
// ============================================================================
self.onmessage = function(e) {
    ...
    try {
        ...
        // ---------------------------------------------------------------
        // STEP — Process game step (dist1/dist2 DECLARED FIRST)
        // ---------------------------------------------------------------
        if (msgType === 'STEP') {
            if (!hc1 || !hc2) {
                self.postMessage({
            ...
            // ✅ DECLARED FIRST — BEFORE ANY USE (FIXES "dist1 is not defined")
            var dist1 = 500;
            var dist2 = 500;
            var damage1 = 0;
            var damage2 = 0;
            ...
            self.postMessage({
                ...
                data: {
                    ...
                    red: reward2 + damageReward2,                    timestamp: Date.now()
                }
            });
        }
        ...
        if (msgType === 'TRAIN') {
            ...
            sendLog('Gen ' + generation + ' | ' + (data ? data.winner : 'UNKNOWN') + ' wins (' + blueWins + '-' + redWins + ')', 'train');
        ...
        // ---------------------------------------------------------------
        if (!msgType) {
            sendLog('Unknown message type: ' + msgType, 'warning');
        }
